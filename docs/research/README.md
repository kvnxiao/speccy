...
| `IMPLEMENTATION-PLAN.md` | Build sequencing and engineering choices not in DESIGN (crate layout, dependencies, git-CLI shell-out, template embedding), the M0 readiness checklist, milestones M0–M9 with task checklists. | Authoritative for build order |
...
| `DECISION-LOG.md` | Decision history, including rejection rationale and pre-rename operation names from the research phase. If it conflicts with DESIGN/TERMINOLOGY, the design docs win. | Historical log |
