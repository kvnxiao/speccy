...
- Fail-before/pass-after control: a `kind: command` evidence request may declare `control: fail_before_pass_after` (the only control value; the vocabulary is closed). At `evidence collect` the controller first executes the same declared command against the run's pinned baseline (`base_commit`) in an isolated temporary git worktree — the live worktree is never reset or mutated to prove the negative — expecting failure, then executes it against the candidate state as normal, expecting success. The recorded control outcome is `passed` (baseline failed and candidate succeeded), `failed` (the baseline passed, so the evidence cannot distinguish before from after, or the candidate failed), or `blocked` (the control could not be established: baseline worktree setup failure, baseline spawn or containment failure, or an isolation cleanup that could not be verified — the leaked path is surfaced in the note). An unavailable baseline environment is `blocked`, never passed and never synthesized as a failure, and a control whose isolation cleanup cannot be verified is never reported as passed. Both executions record exit code, output hash, artifact, and before/after repository identity; the isolation path's cleanup result is stored with the control. Control outcomes are surfaced in the verification packet, and any control that did not pass is called out in the review packet. Planner prose requests this control for high/critical bug-fix requirements; there is no dedicated bug-fix spec type.
...
- Pre-fix failure, when possible: for bug fixes, the evidence path should fail against the baseline branch and pass after the fix. For command evidence this is first-class: declare `control: fail_before_pass_after` on the request and the controller proves it mechanically in an isolated baseline worktree (see "Acceptance Ledger").
