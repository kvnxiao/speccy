...
- **An out-of-band commit parks the run at an `escalated` policy gate.** Escalation commits the in-flight diff as a labeled snapshot; a superseding run reconciles on the same branch; rollback stays the explicit human fallback.
