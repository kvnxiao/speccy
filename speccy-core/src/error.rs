...
impl Finding {
    /// A finding with a code and message but no payload path.
    ...
    /// A finding anchored to a `path` into the offending payload.
    pub fn at(
...
pub struct SpeccyError {
    /// The closed error-code class this failure belongs to.
    pub code: ErrorCode,
    /// Human-readable explanation of the failure.
    pub message: String,
    /// Optional structured findings (e.g. lint findings on a validation error).
    pub details: Vec<Finding>,
}

impl SpeccyError {
    /// A detail-free error with the given `code` and `message`.
...
mod tests {
    ...
    fn error_code_serializes_snake_case() {
        let v = serde_json::to_value(ErrorCode::NotImplemented).expect("serializable");
        assert_eq!(v, serde_json::json!("not_implemented"));
        let v = serde_json::to_value(ErrorCode::NotAGitRepo).expect("serializable");
    ...
    fn ok_envelope_wraps_data() {
        ...
        let env = envelope(&r);
        assert_eq!(
            env.get("ok").expect("ok field present"),
        ...
        assert_eq!(
            env.get("data")
                ...
                .get("spec_ref")
                .expect("spec_ref field present"),
            &serde_json::json!("SPEC-1")
        );
    }
    ...
    fn err_envelope_carries_code_message_details() {
        ...
        assert_eq!(
            ...
            &serde_json::json!("bad draft")
        );
        assert_eq!(
            env.get("error")
                .expect("error field present")
                .get("details")
                .expect("details field present")
                .get(0)
                .expect("details[0] present")
                .get("code")
                .expect("code field present"),
