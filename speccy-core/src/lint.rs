...
#[must_use = "the returned lint findings should be inspected, not discarded"]
pub fn lint_draft(draft: &SpecDraft, policy: &CommandPolicy) -> Vec<Finding> {
    let mut findings = lint_required_sections(draft);
    findings.extend(lint_requirements(draft, policy));
    findings.extend(lint_tasks_and_coverage(draft));
    findings
}

/// 1. Required sections present.
fn lint_required_sections(draft: &SpecDraft) -> Vec<Finding> {
    let mut findings = Vec::new();
    if draft.goal.as_deref().map_or("", str::trim).is_empty() {
...
fn lint_requirements(draft: &SpecDraft, policy: &CommandPolicy) -> Vec<Finding> {
    let mut findings = Vec::new();
    ...
    for req in draft.requirements() {
        ...
        if req.statement.as_deref().map_or("", str::trim).is_empty() {
            findings.push(Finding::at(
                "missing_statement",
                &base,
        ...
        for ev in &req.evidence {
            ...
            match ev.kind_enum() {
                ...
                Some(EvidenceKind::Command) => {
                    let cmd = ev.command.as_deref().map_or("", str::trim);
                    if cmd.is_empty() {
                        findings.push(Finding::at(
                            "missing_command",
                            &ev_path,
...
fn lint_tasks_and_coverage(draft: &SpecDraft) -> Vec<Finding> {
    let mut findings = Vec::new();
...
#[must_use = "the match result determines whether the command is allowed"]
...
fn glob_match(pattern: &str, text: &str) -> bool {
    ...
    while ti < t.len() {
        ...
        if pc.is_some_and(|c| c == '?' || Some(c) == tc) {
            pi += 1;
            ti += 1;
        } else if pc == Some('*') {
            star = Some(pi);
...
mod tests {
    ...
    fn flags_missing_evidence_and_bad_risk() {
        ...
        d.requirements
            .as_mut()
            .expect("clean_draft sets requirements")
            .get_mut(0)
            .expect("clean_draft has one requirement")
            .evidence
            .clear();
    ...
    fn flags_uncovered_requirement_and_unknown_ref() {
        ...
        d.tasks
            .as_mut()
            .expect("clean_draft sets tasks")
            .get_mut(0)
            .expect("clean_draft has one task")
            .requirements = vec!["R9".into()];
        let findings = lint_draft(&d, &CommandPolicy::default());
        let codes: Vec<&str> = findings.iter().map(|f| f.code.as_str()).collect();
    ...
    fn command_policy_flags_unlisted_when_configured() {
        ...
        let evidence = d
            ...
            .evidence
            .get_mut(0)
            .expect("clean_draft's requirement has one evidence entry");
