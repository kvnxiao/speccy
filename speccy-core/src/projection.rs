...
/// One spec draft revision, marked approved once its `approve` decision lands.
#[derive(Debug, Clone)]
pub struct Revision {
    pub id: String,
    pub approved: bool,
...
/// Replayed spec read model: request, revisions, decisions, and status.
#[derive(Debug, Clone)]
...
impl SpecState {
    ...
    #[must_use = "the rebuilt spec state should be inspected or stored"]
    ...
    #[must_use = "returns the revision without side effects"]
    ...
    /// The revision with the given id, if present.
    #[must_use = "returns the revision without side effects"]
...
/// Replayed per-task read model: linked requirements, status, and round.
#[derive(Debug, Clone)]
...
/// Replayed per-requirement runtime: resolution status with its evidence,
/// findings, and notes.
...
/// A recorded worker handoff with its task and round context.
#[derive(Debug, Clone)]
...
/// Replayed run read model: tasks, requirements, evidence, findings, and run
/// state that the directive engine reads from.
#[derive(Debug, Clone)]
...
impl RunProjection {
    ...
    #[must_use = "the rebuilt run projection should be inspected or stored"]
    ...
    #[expect(
        ...
        reason = "single dispatch match over the event vocabulary; splitting would scatter the state machine across helpers with no gain in clarity"
    )]
            ...
            Event::RequirementStatusSet { updates } => {
                for u in updates {
                    ...
                    if !u.evidence.is_empty() {
                        entry.evidence.clone_from(&u.evidence);
                    }
                    if !u.findings.is_empty() {
                        entry.findings.clone_from(&u.findings);
                    }
                    if u.residual_risk.is_some() {
                        entry.residual_risk.clone_from(&u.residual_risk);
                    }
                    if u.note.is_some() {
                        entry.note.clone_from(&u.note);
                    }
                }
    ...
    #[must_use = "returns the task without side effects"]
    ...
    #[must_use = "returns the status without side effects"]
    pub fn req_status(&self, id: &str) -> RequirementStatus {
        self.requirements
            .get(id)
            .map_or(RequirementStatus::Pending, |r| r.status)
    }
    ...
    #[must_use = "returns the accepted-risk check without side effects"]
    ...
    #[must_use = "returns the confirmation check without side effects"]
    ...
    #[must_use = "returns the failing requirement ids without side effects"]
    ...
    #[must_use = "returns the reviewed check without side effects"]
    ...
    #[must_use = "returns the head commit without side effects"]
    ...
    #[must_use = "returns the sequence index without side effects"]
    ...
    #[must_use = "returns the scanned check without side effects"]
    ...
    #[must_use = "returns the blocking findings without side effects"]
    ...
    /// The first still-`queued` task, if any.
    #[must_use = "returns the task without side effects"]
    pub fn next_queued_task(&self) -> Option<&TaskState> {
        self.tasks.iter().find(|t| t.status == TaskStatus::Queued)
    }

    /// Every task has reached `integrated`.
    #[must_use = "returns the done check without side effects"]
    ...
    /// The task currently `building` or `in_review`, if any.
    #[must_use = "returns the task without side effects"]
