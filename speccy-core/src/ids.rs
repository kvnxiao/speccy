...
#[must_use = "generates a new identifier that must be recorded to be useful"]
...
/// an identifier).
#[must_use = "computes a slug that must be used to have any effect"]
...
mod tests {
    ...
    fn spec_ref_has_shape() {
        ...
        assert_eq!(parts.get(1).expect("date part present").len(), 8);
        assert_eq!(parts.get(2).expect("suffix part present").len(), 4);
    }
