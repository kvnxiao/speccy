...
#![expect(
    clippy::expect_used,
    reason = "integration-test helpers assert on known-shape output; expect is the idiomatic way a test fails and never reaches shipped code"
)]

mod common;

use common::Harness;
use speccy_core::config::Persona;
use speccy_core::config::ProjectConfig;
use speccy_core::render::Harness as Target;
use speccy_core::render::render_pack;

fn find<'a>(files: &'a [speccy_core::render::ManagedFile], suffix: &str) -> &'a str {
    files
        .iter()
        .find(|f| f.path.ends_with(suffix))
        .map(|f| f.contents.as_str())
        .expect("managed file with suffix present")
}

#[test]
fn golden_claude_pack() {
    let files = render_pack(Target::Claude, &ProjectConfig::default()).expect("render claude pack");
...
fn golden_codex_pack() {
    let files = render_pack(Target::Codex, &ProjectConfig::default()).expect("render codex pack");
    insta::assert_snapshot!(
...
fn golden_reviewer_with_model() {
    ...
    let claude = render_pack(Target::Claude, &config).expect("render claude pack with model");
    insta::assert_snapshot!(
        "claude-reviewer-model",
        find(&claude, "speccy-reviewer-defects.md")
    );
    let codex = render_pack(Target::Codex, &config).expect("render codex pack with model");
    insta::assert_snapshot!(
