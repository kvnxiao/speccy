...
//! Exercises SPEC-0005 REQ-001..REQ-006 through the binary entry point.
...
fn write_vision(ws: &Workspace, body: &str) -> TestResult {
    let path = ws.root.join(".speccy").join("VISION.md");
...
#[test]
fn greenfield_renders_vision_agents_and_next_spec_id() -> TestResult {
    let ws = Workspace::new()?;
    write_vision(&ws, "# Vision\nBuild a todo app.\n")?;
    ...
    cmd.assert()
        .success()
        .stdout(contains("Build a todo app"))
...
#[test]
fn greenfield_missing_vision_exits_one_with_path_in_stderr() -> TestResult {
    let ws = Workspace::new()?;
    let mut cmd = Command::cargo_bin("speccy")?;
    cmd.arg("plan").current_dir(ws.root.as_std_path());
    cmd.assert()
        .failure()
        .code(1)
        .stderr(contains(".speccy/VISION.md"));
    Ok(())
}
...
#[test]
fn amend_form_inlines_existing_spec_md() -> TestResult {
    let ws = Workspace::new()?;
    write_agents(&ws, "# Agents\nrules\n")?;
    write_spec(
        &ws.root,
