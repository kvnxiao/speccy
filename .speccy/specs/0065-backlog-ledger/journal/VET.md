...
## Invocation 2 — 2026-06-14T04:28:29Z

<drift-review verdict="blocking" round="1" date="2026-06-14T04:28:29Z" model="claude-opus-4-8[1m]">
REQ-008's amend wiring is itself correct and fully covered (existence-guarded `.speccy/BACKLOG.md` stage in amend's step-7 commit, ejected to both hosts, committer-not-producer boundary honored), but adding REQ-008 left the brainstorm body's durability narrative stale: it still hard-names `speccy-plan` as the sole committer of a brainstorm-appended backlog entry, contradicting REQ-008/DEC-002 on the amendment hand-off path.

- REQ-008 / DEC-002 (amend is a committer of an inherited brainstorm append; amend path now durably commits the entry) -> brainstorm step 7 states categorically "the writing skill it hands off to (`speccy-plan`, step 8) sweeps this dirty `.speccy/BACKLOG.md` into its commit", but the same body's step 8 and Exit say the writing skill is `speccy-plan` (new SPEC) OR `speccy-amend` (existing SPEC). On a brainstorm-framed amendment the hand-off is to `speccy-amend`, never `speccy-plan`, so the named mechanism never fires; the entry is actually saved by REQ-008's new amend stage, which the brainstorm narrative does not mention. An agent on the amendment path reading step 7 is told a committer (plan) that is not invoked. This is the precise hand-off REQ-008 was added to close, so the brainstorm prose now describes only the pre-amend (plan-only) intent rather than the final intent. See resources/modules/skills/speccy-brainstorm.md:204-207 (ejected: .claude/skills/speccy-brainstorm/SKILL.md:209 and .agents/skills/speccy-brainstorm/SKILL.md:209). Fix is bounded: generalize the step-7 committer clause to "the writing skill it hands off at step 8 (`speccy-plan` for a new SPEC, `speccy-amend` for an amendment) sweeps this dirty backlog into its commit", then reeject for parity. No functional gap — amend commits correctly regardless — but the narrative is the cross-task inconsistency per-task review structurally missed.
</drift-review>
<holistic-fix verdict="addressed" round="1" date="2026-06-14T04:31:12Z" model="claude-opus-4-8[1m]">
Summary: Generalized brainstorm step-7's committer clause so the amendment hand-off names speccy-amend, resolving the REQ-008/DEC-002 narrative drift.

Addressed:
- REQ-008 / DEC-002 (brainstorm step 7 hard-named `speccy-plan` as the sole committer of a brainstorm-appended `.speccy/BACKLOG.md` entry, but on a brainstorm-framed amendment the step-8 hand-off is to `speccy-amend`, never `speccy-plan` — so the named mechanism never fires and an agent on the amend path is told a committer that is not invoked) -> rewrote the step-7 clause in `resources/modules/skills/speccy-brainstorm.md:204-207` from "the writing skill it hands off to (`speccy-plan`, step 8) sweeps this dirty `.speccy/BACKLOG.md` into its commit" to "the writing skill it hands off to at step 8 (`speccy-plan` for a new SPEC, `speccy-amend` for an amendment) sweeps this dirty `.speccy/BACKLOG.md` into its commit". This matches step 8 and the Exit, which already branch on new-SPEC vs amendment. Re-ejected with parity: `.claude/skills/speccy-brainstorm/SKILL.md:210` renders `/speccy-amend`, `.agents/skills/speccy-brainstorm/SKILL.md:210` renders bare `speccy-amend`.

Side discoveries:
- No functional change: the amend path already commits the inherited entry via REQ-008's existence-guarded stage; this was purely the cross-task prose inconsistency the per-task review structurally missed. Both committer paths (plan for new SPEC, amend for amendment) are now correctly enumerated in the durability narrative.
- Hygiene suite green: `cargo test --workspace`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo +nightly fmt --all --check`, `cargo deny check`.
</holistic-fix>
<drift-review verdict="pass" round="2" date="2026-06-14T04:32:50Z" model="claude-opus-4-8[1m]">
Round-1's lone blocking finding — brainstorm step-7 hard-naming `speccy-plan` as the sole committer of a brainstorm-appended backlog entry, stale against the REQ-008/DEC-002 amend hand-off — is genuinely fixed with ejected parity, the brainstorm durability narrative is now consistent with the actual amend-stages-BACKLOG mechanism, and the fix introduced no new drift.

Round-2+ scrutiny of the prior `<holistic-fix>` (single Addressed claim):
- REQ-008 / DEC-002 (brainstorm step-7 committer clause) -> verified resolved. resources/modules/skills/speccy-brainstorm.md:204-207 now reads "the writing skill it hands off to at step 8 (`speccy-plan` for a new SPEC, `speccy-amend` for an amendment) sweeps this dirty `.speccy/BACKLOG.md` into its commit" — no longer hard-names plan as sole committer. Ejected with parity: .claude/skills/speccy-brainstorm/SKILL.md:208-211 renders `/speccy-amend`, .agents/skills/speccy-brainstorm/SKILL.md:208-211 renders bare `speccy-amend` (expected cmd_prefix divergence, not drift).
- Narrative now matches the real mechanism: the amend body genuinely stages the inherited entry under an existence guard (`test -f .speccy/BACKLOG.md && git add .speccy/BACKLOG.md`) at resources/modules/skills/speccy-amend.md:128-141, ejected to .claude/skills/speccy-amend/SKILL.md:287-302 and .agents/skills/speccy-amend/SKILL.md:287-302. An agent on the amend path reading brainstorm step 7 is now pointed at a committer that actually fires.
- No residual stale committer clause elsewhere in the brainstorm body: the intro (line 9-17) and step-8 routing (line 216-222) name `speccy-plan` only as the dominant new-SPEC hand-off (orientation prose), not as the sole backlog committer; the Exit (line 257-258) already branches plan/amend. Consistent.

No scope creep: round-2 change is prose-only; the only Rust in the whole diff remains the two init tests vetted in invocation 1. DEC-001 convention-only posture intact (CLI never touches the backlog); amend is committer-not-producer per REQ-008/DEC-002, so the producer set is unchanged. No non-goal violated. REPORT.md staleness noted as out of scope per the caller (refreshed separately) — not assessed.
</drift-review>
<gate verdict="passed" tasks_hash="662a1b5e77259e54f0809bc1add31537ce1004e6182b922dbe40f9242b2d734a" date="2026-06-14T04:33:04Z">
Post-amend re-check (REQ-008): round-1 caught stale brainstorm step-7 committer prose (named plan as sole committer; amend hand-off uncovered), fixed and reejected; round-2 confirmed pass with parity. No new whole-SPEC drift.
</gate>
