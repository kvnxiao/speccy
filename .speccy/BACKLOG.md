...
F-11: Each artifact created by the previous step should ideally be locked in place afterwards. Today, we have a hashing mechanism to lock a SPEC.md in place when TASKS.md are decomposed and ready for implementation. We need something similar for TASKS.md when the loop is done implementing, and when the loop is done reviewing, otherwise the TASKS.md file will grow in size during each loop iteration (and each loop iteration risks overwriting changes made by other agents, on top of accidentally deleting parts of the file with non-zero possibility). Implementer adds implementer notes, and reviewers add reviewer notes to TASKS.md which makes it blow up in size approximately 2-3x the original size of the decomposed TASKS.md file. Another thing is traceability of the implementer and reviewer notes, we should add which model accomplished and generated the artifact, and when - today we only report the status. We should probably also consolidate on the XML attribute formatting for <implementer-notes> and <review>. Today, implementer notes record just a single session="session-0032-t002-2026-05-19", but we should standardize on having an ISO8601 formatted `date`, `model` (e.g. model="claude-opus-4.7[1m]"), and an optional retry-num="1" for retries. For reviewer notes, we should also standardize on the XML attribute formatting with `date` and `model` on top of the existing `persona` and `verdict`.
