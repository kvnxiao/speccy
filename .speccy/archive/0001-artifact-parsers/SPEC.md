...
- Markdown fenced code blocks containing example REQ headings.
  `docs/ARCHITECTURE.md` is itself a worked example -- the design doc
...
The parsing stack matches `docs/ARCHITECTURE.md`'s "Operational details
...
**Status:** Accepted (per `docs/ARCHITECTURE.md` "Operational details")
...
the robustness `docs/ARCHITECTURE.md` specifically calls out.
</decision>
...
one), as the initial `docs/ARCHITECTURE.md` sketch did. The bidirectional
...
replacements in one place. `docs/ARCHITECTURE.md` is amended in the
...
- [x] Where does `AGENTS.md` live? -- At the project root (existing
  location). `docs/ARCHITECTURE.md`'s file-layout sketch showing
  `.speccy/AGENTS.md` is conceptual. Every host already loads
  `AGENTS.md` from the project root, and the symlink from
  `CLAUDE.md` reinforces that. `docs/ARCHITECTURE.md` should be amended
...
| Date       | Author       | Summary |
|------------|--------------|---------|
| 2026-05-11 | human/kevin  | Initial draft from `docs/ARCHITECTURE.md` decomposition (bootstrap of speccy itself). |
