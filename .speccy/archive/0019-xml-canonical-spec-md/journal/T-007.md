...
- Completed: Rewrote `docs/ARCHITECTURE.md` to remove per-spec
...
- Undone: AGENTS.md was deliberately left alone — it does not
  ...
  `docs/ARCHITECTURE.md`), and the prompt's escape clause
...
(bullet 1) walks every `spec.toml` line in `docs/ARCHITECTURE.md`
...
`docs/ARCHITECTURE.md:1610` (the "What We Deliberately
...
`docs/ARCHITECTURE.md` contains a line covering both
...
- Completed: Added the fifth assertion
  ...
  `docs/ARCHITECTURE.md` and asserts at least one line
  contains both `speccy fmt` and `DEC-003`. Today the
  `docs/ARCHITECTURE.md:1610` "What We Deliberately Don't Do"
