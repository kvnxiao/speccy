...
use serde_json::json;
use speccy::config::ProjectConfig;
...
fn crash_matrix_run_next_is_idempotent_at_every_phase() {
    ...
    for _ in 0..40 {
        ...
        let lease = d2["lease"]["token"].as_str().unwrap().to_string();
        match d2["action"].as_str().unwrap() {
            "claim_task" => {
                let task = d2["subject"]["task"].as_str().unwrap();
                h.ctl(&[
            ...
            "dispatch_worker" => {
                let task = d2["subject"]["task"].as_str().unwrap().to_string();
                let round = d2["round"]["current"].as_u64().unwrap();
        ...
        let handles: Vec<_> = (0..2)
            .map(|_| {
                ...
                scope.spawn(move || {
                    ...
                    out["evidence"][0]["exit_code"].as_i64().unwrap()
                })
            })
            .collect();
        handles.into_iter().map(|h| h.join().unwrap()).collect()
...
fn golden_all_managed_files() {
    for target in [Target::Claude, Target::Codex] {
        let files = render_pack(target, &ProjectConfig::default()).unwrap();
        for f in &files {
            let name = format!(
                "{}--{}",
                target_key(target),
