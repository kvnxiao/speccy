# Speccy Spec Card Example

Read this before presenting the approval card. The card is the human decision
surface: it should answer what changes, what does not change, how Speccy will
know it worked, and what could go wrong.
...
Open questions:
- Staging email delivery is not available locally. Recommended answer: verify
  local token creation in this run and leave live delivery as manual evidence.
...
- Lead with the recommended next action and canonical replies.
- Show distinct `kind: command` evidence strings exactly as they will run.
- Keep acceptance statements plain enough for a human to approve.
- Mention open questions only when they must remain unresolved before approval;
  include the recommended answer.
- Collapse internal IDs into the footer except for the spec ref and command
  strings the human is approving.
