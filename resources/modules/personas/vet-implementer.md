...
`{{ cmd_prefix }}speccy-vet` skill) passes you a list of
...
- `.speccy/specs/NNNN-slug/journal/VET.md` — this is the
  holistic-loop journal. The `{{ cmd_prefix }}speccy-vet`
...
- `model` — required. The slash-suffix on the model string encodes
  ...
  `claude-opus-4.8[1m]/low`); hosts without an effort knob omit
  the suffix.
